...
const hrDirectorRegistrationSchema = z.object({
...
type HrDirectorRegistrationValues = z.infer<typeof hrDirectorRegistrationSchema>;

export default function RegisterHrDirectorPage() {
  const { t } = useLanguage();
  const { user, loading: authLoading, signupHrDirector } = useAuth();
  const { toast } = useToast();
  const router = useRouter();
  const [isSubmitting, setIsSubmitting] = React.useState(false);

  const form = useForm<HrDirectorRegistrationValues>({
    resolver: zodResolver(hrDirectorRegistrationSchema),
    defaultValues: {
      name: "",
      email: "",
      password: "",
  ...
  async function onSubmit(data: HrDirectorRegistrationValues) {
    setIsSubmitting(true);
    try {
      await signupHrDirector({ 
        name: data.name,
        email: data.email,
      ...
      toast({
        title: t('hrDirectorRegisteredTitle'),
        description: t('hrDirectorRegisteredMessage', { email: data.email }),
      });
      form.reset();
    } catch (error: any) {
      console.error("HR Director registration failed:", error);
    ...
    <div className="container mx-auto py-8 px-4 max-w-lg">
      <Card className="shadow-xl">
        <CardHeader className="text-center space-y-1">
          ...
          <CardTitle className="text-2xl">{t('registerHrDirectorTitle')}</CardTitle>
          <CardDescription>{t('registerHrDirectorDescription')}</CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
