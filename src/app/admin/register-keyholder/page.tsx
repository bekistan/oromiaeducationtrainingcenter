...
type KeyholderRegistrationValues = z.infer<typeof keyholderRegistrationSchema>;

export default function RegisterKeyholderPage() {
  const { t } = useLanguage();
  const { user, loading: authLoading, signupKeyholder } = useAuth();
  const { toast } = useToast();
  const router = useRouter();
  const [isSubmitting, setIsSubmitting] = React.useState(false);

  const form = useForm<KeyholderRegistrationValues>({
    resolver: zodResolver(keyholderRegistrationSchema),
    defaultValues: {
      name: "",
      email: "",
      password: "",
  ...
  async function onSubmit(data: KeyholderRegistrationValues) {
    ...
    try {
      ...
      toast({
        title: t('keyholderRegisteredTitle'),
        description: t('keyholderRegisteredMessage', { email: data.email }),
      });
      form.reset();
    } catch (error: any) {
      console.error("Keyholder registration failed:", error);
    ...
    <div className="container mx-auto py-8 px-4 max-w-lg">
      <Card className="shadow-xl">
        <CardHeader className="text-center space-y-1">
          ...
          <CardTitle className="text-2xl">{t('registerKeyholderTitle')}</CardTitle>
          <CardDescription>{t('registerKeyholderDescription')}</CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
